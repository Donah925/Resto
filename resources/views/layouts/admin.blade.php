...
<head>
    ...
    <title>@yield('title', 'Espace Admin') - RestoApp</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-gray-900 text-white flex-shrink-0 hidden md:block">
            <div class="p-6">
                ...
                <p class="text-gray-400 text-sm mt-1">Admin</p>
            </div>

            <nav class="mt-6">
                <x-sidebar-link href="{{ route('admin.dashboard') }}" icon="📊" :active="request()->routeIs('admin.dashboard')">
                    Tableau de bord
                </x-sidebar-link>

                <p class="px-6 pt-4 pb-2 text-xs uppercase text-gray-500">Gestion</p>
                <x-sidebar-link href="{{ route('admin.restaurants.index') }}" icon="🏪" :active="request()->routeIs('admin.restaurants.*')">
                    Restaurants
                </x-sidebar-link>
            </nav>
        </aside>
